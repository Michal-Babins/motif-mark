...
with open(input_file, "r") as fa:
    for line in fa:
        ...
        if counter == 1:
            line = line.strip()
            header.append(line) #locate first header as start
            continue
        elif (not line.startswith(">") or "\n" not in line):
            line = line.strip("\n")
            seq.append(line) #locate and add sequences to seq list
        else:
            line = line.strip("\n")
            header.append(line) #add the rest of the headers to the list
            conc = ""
            for i in seq:
                conc += str(i)
            total_seq.append(conc) #gather all string values of sequence
            seq = [] #clear seq
    total_seq.append(conc)
    for i in range(len(header)):
        #set header as keys and seq as values to make them equivalent to on another
        my_dict[header[i]] = (total_seq[i])
